...
export function HeroSection({ summary, dailyWords }: HeroSectionProps) {
  ...
  return (
    <section className="relative overflow-hidden pt-10 sm:pt-14">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(125,211,252,0.18),transparent_32%),radial-gradient(circle_at_80%_10%,rgba(244,114,182,0.14),transparent_24%),linear-gradient(180deg,#ffffff_0%,#f8fafc_55%,#ffffff_100%)]" />
      <PageShell className="relative">
        <div className="grid items-end gap-12 py-16 lg:grid-cols-[1.15fr_0.85fr] lg:py-24">
          <motion.div style={{ y }} className="space-y-8">
            <div className="inline-flex items-center rounded-full border border-black/8 bg-white/70 px-4 py-2 text-sm text-zinc-600 backdrop-blur-xl">
              Premium English vocabulary, calibrated for Arabic learners
            </div>
            <div className="space-y-6">
              <h1 className="max-w-4xl text-5xl font-semibold tracking-tight text-zinc-950 sm:text-6xl md:text-7xl">
          ...
          <div className="overflow-hidden rounded-[2.2rem] border border-black/5 bg-white/72 p-6 shadow-[0_30px_120px_rgba(15,23,42,0.08)] backdrop-blur-2xl">
            <div className="text-sm font-semibold tracking-[0.2em] text-zinc-400 uppercase">
              Daily words
            </div>
            <div className="mt-4 text-2xl font-semibold tracking-tight text-zinc-950">
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[1.7rem] border border-black/5 bg-white/72 p-5 backdrop-blur-xl">
